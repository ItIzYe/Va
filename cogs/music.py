...
class MusicPlayer(commands.Cog, name='Music'):
    ...
    async def queue(self, msg, song):
        ...
        return await msg.send(f"**{title} wurde zur Warteschlange hinzugefügt**".title())
    ...
    async def start_song(self, msg, song):
        ...
        emb = discord.Embed(colour=self.random_color, title='Wird jetzt gespielt:',
                            description=download.title, url=download.url)
        emb.set_thumbnail(url=download.thumbnail)
        emb.set_footer(
            text=f'Angefragt von {msg.author.display_name}', icon_url=msg.author.avatar_url)
        loop = asyncio.get_event_loop()
    ...
    async def before_play(self, msg):
        ...
        if msg.author.voice is None:
            return await msg.send('**Bitte trete zuerst einem Sprachkanal bei**'.title())
        ...
        if msg.voice_client.channel != msg.author.voice.channel:
            ...
            if self.player[msg.guild.id]['queue']:
                ...
                return await msg.send("Bitte trete dem Sprachkanal des Bots bei um diesen Command nutzen zu können")
    ...
    async def repeat(self, msg):
        ...
        if msg.guild.id in self.player:
            ...
            return await msg.send("Es wird keine Audio abgespielt")
        return await msg.send("Bot ist in keinem Sprachkanal oder spielt Musik")
    ...
    async def reset(self, msg):
        ...
        """
        if msg.voice_client is None:
            return await msg.send(f"**{msg.author.display_name}, der Bot spielt zurzeit keine Musik ab.**")

        if msg.author.voice is None or msg.author.voice.channel != msg.voice_client.channel:
            return await msg.send(f"**{msg.author.display_name}, du musst im gleichen Sprachkanal wie der Bot sein.**")

        if self.player[msg.guild.id]['queue'] and msg.voice_client.is_playing() is False:
            return await msg.send("**Kein Song im Shuffle oder in der Warteschlange**".title(), delete_after=25)
    ...
    async def skip(self, msg):
        ...
        """
        if msg.voice_client is None:
            return await msg.send("**Es wird keine Musik gespielt**".title(), delete_after=60)

        if msg.author.voice is None or msg.author.voice.channel != msg.voice_client.channel:
            return await msg.send("Bitte trete dem Sprachkanal des Bots bei")

        if not self.player[msg.guild.id]['queue'] and msg.voice_client.is_playing() is False:
            return await msg.send("**Kein Song zum überspringen**".title(), delete_after=60)
    ...
    async def stop(self, msg):
        ...
        """
        if msg.voice_client is None:
            return await msg.send("Bot ist mit keinem Sprachkkanal verbunden")

        if msg.author.voice is None:
            return await msg.send("Du musst im gleichen Sprachkanal wie der Bot sein")
        ...
        if msg.author.voice is not None and msg.voice_client is not None:
            ...
            return await msg.send(f"**{msg.author.display_name}, Es wird kein Song gespielt oder ist in der Warteschlange**")
    ...
    async def leave(self, msg):
        ...
        if msg.author.voice is None:
            return await msg.send("Du musst in dem gleichen Sprachkanal wie der Bot sein, um diesen Command benutzen zu können")
    ...
    async def pause(self, msg):
        ...
        if msg.author.voice is not None and msg.voice_client is not None:
            if msg.voice_client.is_paused() is True:
                return await msg.send("Song wurde schon pausiert")
    ...
    async def resume(self, msg):
        ...
        if msg.author.voice is not None and msg.voice_client is not None:
            if msg.voice_client.is_paused() is False:
                return await msg.send("Song wird schon gespielt")

            if msg.voice_client.is_paused() is True:
                msg.voice_client.resume()
                return await msg.message.add_reaction(emoji='✅')

    @commands.command(name='queue', aliases=['song-list', 'q', 'current-songs'])
    async def _queue(self, msg):
        ...
        if msg.voice_client is not None:
            if msg.guild.id in self.player:
                if self.player[msg.guild.id]['queue']:
                    ...
                    emb.set_footer(
                        text=f'Command benutzt von {msg.author.name}', icon_url=msg.author.avatar_url)
        ...
        return await msg.send("Kein Song in der Warteschlange")

    @commands.command(name='song-info', aliases=['song?', 'nowplaying', 'current-song'])
    async def song_info(self, msg):
        ...
        if msg.voice_client is not None and msg.voice_client.is_playing() is True:
            emb = discord.Embed(colour=self.random_color, title='Spielt gerade:',
        ...
        return await msg.send(f"**Kein Song in der Warteschlange**".title(), delete_after=30)

    @commands.command(aliases=['move-bot', 'move-b', 'mb', 'mbot'])
    async def join(self, msg, *, channel: discord.VoiceChannel = None):
        ...
        """
        if msg.voice_client is not None:
            return await msg.send(f"Der Bot ist schon in einem Sprachkanal\nMeintest du {msg.prefix}moveTo")
    ...
    async def before_join(self, msg):
        if msg.author.voice is None:
            return await msg.send("Du bist in keinem Sprachkanal")
    ...
    async def volume(self, msg, vol: int):
        ...
        return await msg.send("**Bitte trete erst einem Sprachkanal bei**".title(), delete_after=30)
    ...
    async def download(self, ctx, *, song):
        ...
        try:
            with youtube_dl.YoutubeDL(ytdl_download_format_options) as ydl:
                ...
                embed = discord.Embed(
                    title="Dein Download ist fertig", description="Bitte warte einen Moment")
                await ctx.send(embed=embed, delete_after=30)
                await ctx.send(file=discord.File(filename))
                os.remove(filename)
        except (youtube_dl.utils.ExtractorError, youtube_dl.utils.DownloadError):
            embed = discord.Embed(title="Song konnte nicht heruntergeladen werden...", description=("Song:"+song))
            await ctx.send(embed=embed)

    @volume.error
    async def volume_error(self, msg,error):
        if isinstance(error, commands.MissingPermissions):
            return await msg.send("Dir fehlen leider die Berechtigungen für den Command :/", delete_after=30)
